...
function verifyCode(webhookPath, expectedType) {
  return [
    ...
    "const rawSecret = $env.AUTOMATION_SERVICE_SECRET;",
    "if (!rawSecret || typeof rawSecret !== 'string' || rawSecret.trim() === '') {",
    "  throw new Error('Unauthorized: Missing or empty AUTOMATION_SERVICE_SECRET');",
    "}",
    "const secret = rawSecret.trim();",
    "const lowerSecret = secret.toLowerCase();",
    "if (secret.length < 32 || lowerSecret === 'placeholder' || lowerSecret === 'changeme' || lowerSecret === 'secret' || lowerSecret.includes('placeholder') || lowerSecret.includes('changeme')) {",
    "  throw new Error('Unauthorized: Insecure AUTOMATION_SERVICE_SECRET configured');",
    "}",
